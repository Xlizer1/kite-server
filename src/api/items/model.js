    ...
    try {
        const sql = `
            SELECT
                ...
                (SELECT COUNT(*) FROM items 
                 ...
                 AND restaurant_id = ?) AS total_items
            FROM items i
        ...
        return {
            items: result.map(item => ({
                ...
                total_pages: Math.ceil(item.total_items / limit),
    ...
    try {
        ...
        if (uploadedFile) {
            ...
            if (!allowedTypes.includes(uploadedFile.mimetype)) {
                throw new CustomError(`Invalid file type for image: ${uploadedFile.originalname}. Allowed types: ${allowedTypes.join(', ')}`, 400);
            }
